...
% full
P.Ap  = A/16 - A'/32 - eye(n)/16;   	% amplitude coupling
P.Ar  = A/16 + A'/32;                   % phase coupling

% phase only
% P.Ap  = A/64 - A'/32 - eye(n)/16;     % amplitude coupling
% P.Ar  = A/16 + A'/32;             	% phase coupling
...
%--------------------------------------------------------------------------
f = @(x,v,P) [P.Ap.*cos(bsxfun(@minus,x.p,x.p'))*(x.r - P.r)...
    + P.C*v.*cos(x.p);
    sum(P.Ar.*exp(-abs(bsxfun(@minus,x.r,x.r'))).*...
...
pE.Ar    = zeros(n);                    % set prior phase and amplitude 
pE.Ap    = -speye(n)/16;                % coupling parameters to 0
...
DEM.M(1).g  = g;
DEM.M(1).pE = pE;
DEM.M(1).pC = diag(spm_vec(pC))*(1/8);
DEM.M(1).V  = exp([Vr Vp]);   
